...
func verifyRegistryBinding(registryPath, recordID, root, packageRoot string) error {
	...
	record, err := registryRecordFromDoc(doc, recordID)
	if err != nil {
		return err
	}
	if err := verifyRegisteredTargetIdentity(record); err != nil {
	...
	return verifyRegistryBindingRecord(record, root, packageRoot)
}

// registryRecordFromDoc 在文档已加载时查找并校验目标 record，供同一验证链
// 复用一份 LoadRegistry 结果。
func registryRecordFromDoc(doc RegistryDocument, recordID string) (RegistryRecord, error) {
	for _, record := range doc.Records {
		if record.ID == recordID {
			if !strings.EqualFold(record.Status, "active") || !validAdmissionRegistryRecord(record) {
				return RegistryRecord{}, fmt.Errorf("UNREGISTERED_INSTALL: registry record %q is inactive or incomplete", recordID)
			}
			return record, nil
		}
	}
	return RegistryRecord{}, fmt.Errorf("UNREGISTERED_INSTALL: registry record %q is missing", recordID)
}

// verifyRegistryBindingRecord 校验 record 与 workflow root/package root 及启
// 动器身份的绑定关系。不含 registry 加载与安装目标摘要核验：二者由调用方
// 在上游完成，避免同一验证链内重复读盘与重复全树哈希。
func verifyRegistryBindingRecord(record RegistryRecord, root, packageRoot string) error {
