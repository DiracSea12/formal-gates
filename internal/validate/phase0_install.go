...
func runInstalledBinarySmokeWithPolicy(path string, allowPlaceholder bool) error {
	...
	// 安装后 smoke 挂起会把整个安装事务拖住，直到锁的 ModTime 陈旧阈值把
	// 仍在存活的持锁者误判抢占。给 smoke 一个明确超时，让挂起以确定性失败
	// 进入既有回滚路径，而不是依赖陈旧阈值兜底。
	ctx, cancel := context.WithTimeout(context.Background(), installedBinarySmokeTimeout)
	defer cancel()
...
// installedBinarySmokeTimeout 是 installed binary --version smoke 的上限。
// 正常 smoke 即时完成；该上限只为把挂起故障转化为确定性失败。
const installedBinarySmokeTimeout = 2 * time.Minute

// lockHeartbeatInterval 是持锁期间刷新锁文件 ModTime 的周期，取 ModTime
// 陈旧阈值的十分之一，保证慢事务期间锁不会被判 stale。
const lockHeartbeatInterval = 1 * time.Minute
...
func acquirePhase0Lock(path, description string) (func(), error) {
	...
	for {
		...
		if err == nil {
			...
			// 持锁期间周期性刷新锁文件 ModTime（心跳）。陈旧判定以 ModTime 超过
			// 阈值为据，持锁者若长时间不刷新，慢事务（如大包 copy + smoke）会被
			// 另一个入口按 stale 抢锁、破坏互斥；心跳让存活持有者始终“年轻”。
			// 心跳只在锁文件内容仍归本进程时刷新，不给抢占者的新锁续命。
			stop := make(chan struct{})
			go lockHeartbeat(path, stop)
			return func() { close(stop); _ = os.Remove(path) }, nil
		}
		if !os.IsExist(err) {
			return nil, err
		}
...
// lockHeartbeat 在持锁期间每 lockHeartbeatInterval 刷新一次锁文件的
// ModTime。只有锁文件内容仍是本进程的 pid 时才刷新：锁被抢占/删除后心跳
// 自行退出，不会误刷别人的锁。stop 关闭即退出。
func lockHeartbeat(path string, stop <-chan struct{}) {
	...
	for {
		...
		case <-ticker.C:
			data, err := os.ReadFile(path)
			if err != nil || strings.TrimSpace(string(data)) != fmt.Sprintf("pid=%d", os.Getpid()) {
				return
			}
			now := time.Now()
