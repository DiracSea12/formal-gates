...
**一套基于实现者、审查者与测试者强制分离的轻量开发审查工作流：从需求澄清到封板，每步都有标准化流程、门禁与记录**
...
formal-gates 是一套完整的 AI 辅助开发工作流：需求澄清、路由选择、QA设计、开发、
...
- **一套完整流程** —— 需求澄清、路由选择、QA 先设计、开发、快照、独立审查、返修、封板，一步不缺、每步留记录。
...
- **门与提示词都可自定义** —— 门的审查逻辑在 `gates/*.md`，各动作（需求澄清、QA 设计/审查/执行、开发 worker、继承判定）的提示词在 `prompts/actions/*.md`，都是安装包里的普通 Markdown 文件。通过修改提示词文件，每道门怎么审，审哪些内容，你可以完全自定义，包括预制的门。审查要怎么做，全权由你来定。
...
一次正式 run 从需求澄清到封板，按下面顺序推进。**门的集合是动态的**：`gates/*.md` 里的每个文件就是一道门，加文件就多一道、删文件就少一道，不在流程里写死。

1. **启动** —— 选定仓库的 VCS（Git / SVN / P4），冻结基线，登记需求与需求文档。
2. **需求澄清登记** —— 登记已确认的需求与方案，记录 PASS；这是后续所有判定的唯一依据。
...
开新窗口解决"独立零上下文审查"这一点。formal-gates 提供的是完整流程：需求澄清、确认、QA 先设计、开发、快照、独立审查、返修、封板，每次必走、每步留记录、返修只重测变化的用例。
