...
聊天结论、Markdown 标签和开发者自报都不能产生正式 PASS。独立 QA 执行者先产出完整结果和 case binding，主代理只写包含五个证据引用的 schema-version-2 `QA_EXECUTION`；workflow 命令在原子替换状态前校验批准用例、QA 结果、binding、hash、snapshot 和证据闭包。
...
缺少批准用例或 QA 自有结果、case 覆盖不完整、结果失败、binding 错误、路径或 hash 不对、snapshot 过期、schema version 1、未知字段或仅有 Markdown 时，命令拒绝且不改变权威状态。QA Execution 不需要第二个 reviewer 或 receipt。
