...
1. 收到创建、编辑、移动或删除项目内容的请求时，按常规方式直接处理、不自动触发受
   理；仅当需求过大、过复杂（规模、耦合、风险、验证复杂度足以超过普通直接处理的能
   力）时，提醒用户可选择触发 formal-gates 正式流程——一次性提及、不要求用户回应、
   不得反复提及、不得自行触发。非修改性提问、解释、诊断、review 不触发。
...
**默认直接处理分支**：普通请求直接按常规方式开发，不建 run、不问询、不审；大需求提
醒一次后用户不回应，也按常规方式直接处理。已有正式 run 进行中时的行为由主代理按情
境决定。
