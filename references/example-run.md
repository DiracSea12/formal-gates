...
1. 收到请求后，先判断它是否属于「创建、编辑、移动或删除项目内容」——是，触发受理
   流程。非修改性提问、解释、诊断、review 不触发。
2. 本会话第一次触发时，照读 `SKILL.md` 开发受理流程中的【流程提示】书面提示（前缀
   必须保留；用户以其他语言沟通时用对等书面表述），显式提醒用户可作会话级「不走流
   程」声明。
3. 本步不读 `references/formal-flow.md`——受理阶段只决定轻量或正式，命令形式后面才
   需要。现在只按 `SKILL.md` 开发受理流程操作。

**不走流程分支**：用户声明「不走流程」后，本会话所有内容修改不建 run、不问询、不
审，直接按常规方式开发，其余步骤全部跳过。该声明可随时提出或撤销；已有正式 run 进
行中时声明，不预设固定行为、由主代理按情境决定。

**走正式流程分支**：用户未声明，默认按正式流程执行。继续。
...
5. 用户明确确认后，评估工作量——规模、耦合、风险、验证复杂度——并呈现开发流程选
   择。需求确认与路线选择不得合并。

## 3. 选择轻量/正式

1. 呈现「轻量 / 正式」并给推荐、解释理由。本例请求跨两个子命令、改数据布局、需要
   验证，推荐「正式」。
2. 路线（`full`/`custom`）不在这里确认——它在正式流程的拆分决定之后确认，单一 run
   确认一次。本阶段只定「轻量」或「正式」。
3. 用户选「正式」：把已确认的完整需求与技术方案持久化到环境中任一稳定文档格式（本
   例写一个需求文件），再进入正式流程。
4. 用户选「轻量」则到此为止：不建流程状态、不建正式任务切片，直接按 vibe coding 开
   发，其余步骤全部跳过。
...
4. 发现项分级 P0/P1/P2，是候选输入不是终态。逐项转达用户处置：用户接受 → 该发现项
...
3. 每个门都用全新零上下文审查者，`claim-dispatch` 认领。门任务 = `prompts/reviewer-base.md`
   ...
   时错误中止派发。
4. QA 只用已批准用例。黑盒 QA（LIVE）开发前由独立代理设计、经 `qa-review` 批准；白
   盒 QA（STATIC）开发后由独立代理读实现设计并执行。QA 审查者输入 = 已确认需求 + CLI
   组装的完整候选用例集，不含生产代码、diff、测试、开发者解释、其他审查者结论。
...
2. 出现 QA FAIL 或 P0/P1 门发现项时，整个轮次退回修复，此时同轮的 P2 一并处理。QA
...
2. PENDING 直接阻塞 Seal；RUNTIME_ERROR 需要用户手动跳过；QA FAIL 或 P0/P1 必须先修
   ...
   `--user-requested`）。只有 P2 的 PASS 建议保持可见、不阻塞 Seal。
