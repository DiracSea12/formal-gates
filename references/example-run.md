...
1. 收到创建、编辑、移动或删除项目内容的请求时，默认提醒一次：若需走 formal-gates 流程，
   ...
   能力）时，在默认提醒之外额外强调一次：检测到复杂需求，建议走 formal-gates 流程。两
   次结构化提及是设计上限：不得反复提及、不得自行触发。非修改性提问、解释、诊断、review
   不触发。
...
方式直接开发，不建 run、不问询、不审；大需求额外强调一次（「检测到复杂需求，建议走
formal-gates 流程」）后用户仍不选择，也按常规方式直接处理。已有正式 run 进行中时的行为
由主代理按情境决定。
