...
2. 用户明确要求走正式流程（或明确要求触发 formal-gates）时，照读 `SKILL.md` 开发受
   理流程操作。
3. 本步不读 `references/formal-flow.md`——命令形式在正式流程启动时才需要。现在只按
   `SKILL.md` 开发受理流程操作。
...
5. 用户明确确认后进入正式流程。路线 full / custom 在拆分决定之后确认，lightweight 不
   经拆分决定与路线选择。

## 3. 进入正式流程

1. 用户明确提出 formal-gates 后，完成需求澄清和完整方案确认，随后把已确认的完整需求
   与技术方案持久化到环境中任一稳定文档格式（本例写一个需求文件）。
2. 如果用户未明确模式，在正式流程启动时选择 `lightweight` 或 `regular`；`full`/`custom`
   在拆分决定之后确认。普通请求不进入本示例的正式流程分支，按默认提醒后的常规方式处理。
