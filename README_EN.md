...
- **One default reminder** — on content-modification requests the main agent reminds you once by default: you may directly request the formal-gates flow. Large or complex requests get one additional emphasis (detected complex requirements — consider the formal-gates flow). The main agent never self-triggers and never repeats the mention beyond the two designed ones. Only on your explicit request does it enter the formal flow, where you choose how complete: full, custom, or lightweight.
...
> **The entire flow runs only when you explicitly ask for it.** The main agent never self-triggers: on content-modification requests it reminds you once by default (you may directly request the formal-gates flow), and large or complex requests get one additional emphasis (detected complex requirements — consider the formal-gates flow); it only enters the formal flow on your explicit request. The nine-phase formal flow is heavier because every step carries gates and records; record-only tasks can take the lightweight route (start → register requirement → Seal, no verification) instead of the nine phases.
...
**What happens** — intake starts only when you explicitly ask to run the formal flow. The main agent looks at the current state, then asks about each consequential decision one at a time (in plain language, explaining the consequences of each option), and presents the integrated plan for your explicit confirmation. After confirmation it assesses the workload and asks you whether to enter the formal flow (yes / no); the route (full / custom) is confirmed inside the formal flow, after the slicing decision, while lightweight skips the slicing decision and route selection. On ordinary content-modification requests the main agent reminds you once by default (you may directly request the formal-gates flow); large or complex requests get one additional emphasis (detected complex requirements — consider the formal-gates flow).
...
**In one line** — every request gets one default reminder that you may directly request the formal flow; large or complex requests get one extra emphasis; only on your explicit request does the formal flow run, where full / custom / lightweight then determines completeness (see phase 3).
...
| ZCode | `~/.zcode/skills/formal-gates` | `.zcode/skills/formal-gates` under the selected project |

Installation merges formal-gates' own host hooks into the host config: Claude Code's `~/.claude/settings.json`, Codex's `~/.codex/hooks.json`, and Cursor's `~/.cursor/hooks.json` (project-level installs write the corresponding files under the selected project); a DeepSeek Harness global install writes `$DSH_HOME/cordis.patch.yml` and mounts the bundled minimal Cordis plugin, while a project install writes only the skill and `AGENTS.md` instructions (DSH auto-loads only the home-level patch); a ZCode global install writes `~/.zcode/cli/config.json`, while a project install writes only the skill and `AGENTS.md` instructions (current ZCode ignores project-level hook configuration). Existing non-formal-gates hooks and plugin rows are preserved; when multiple hosts share a project `AGENTS.md`, the managed marker is cleaned only after the last active owner is uninstalled.
...
uses `.cursor/rules/formal-gates.mdc`, DeepSeek Harness uses `$DSH_HOME/AGENTS.md` or project
`AGENTS.md`, and ZCode uses `~/.zcode/AGENTS.md` or project `AGENTS.md`. Cursor global installs do not create a rules file; they
...
1. **Install** — set it up for one of your AI hosts (claude, codex, cursor, dsh, or zcode) as described above.
...
**When is formal-gates triggered?**
The main agent never self-triggers: on content-modification requests it reminds you once by default (you may directly request the formal-gates flow); large or complex requests get one additional emphasis (detected complex requirements — consider the formal-gates flow). Only on your explicit request does it enter the formal flow; intake decides only whether to enter the formal flow (yes / no), and how complete the run is (full / custom) is confirmed inside the formal flow, after the slicing decision; lightweight skips the slicing decision and route selection and is declared at start.
...
Building from source needs Go 1.22+ and one host: claude, codex, cursor, dsh, or zcode. A formal run needs a Git, SVN, or P4 repository; projects without a VCS don't enter the formal flow.
