...
The main agent never self-triggers: on content-modification requests it reminds you once by default (you may directly request the formal-gates flow); large or complex requests get one additional emphasis (detected complex requirements — consider the formal-gates flow). Only on your explicit request does it enter the formal flow; how complete the run is (full / custom) is confirmed inside the formal flow, after the slicing decision; lightweight skips the slicing decision and route selection and is declared at start.
