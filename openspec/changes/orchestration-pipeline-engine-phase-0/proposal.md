...
阶段 0 交付“固定 stable driver + 阶段候选隔离验证”的基础：安装输入可验证、入口可登记、非幂等安装副作用可恢复；对于未来版本化 engine/candidate surface，缺失或不匹配 `stateSchemaVersion`、`workflowDefinitionVersion` 时在写前返回 `UNSUPPORTED_RUN_VERSION`；stable driver 与既有 legacy run 继续当前 state 格式和写入语义，不迁移、不回写。实际 installed binary 的验证证据绑定不可变候选 identity。
