...
1. **批次计划义务**：任何 run 的开发在受理确认时必须给出显式批次计划，作为任务清单的组织结构，随需求一并交用户确认并受产品审/技术审覆盖；允许"单批 + 理由"退化形态。批次划分依据依赖链、体量与风险：依赖链（消费链）必须串行；零耦合支线可并行；共享文件、接口或语义的工作面不并行。接口冻结点优先作为批边界。
2. **批粒度**：批是任务单元，单批一个内聚关注点——语义判据为"能用一句话说清本批 diff 即一批"；批内提交保持原子、可独立构建。批内每次编辑后执行本机轻量检查（构建/vet/lint），全量测试在批边界；每批终态可判——树可构建、全量测试通过、自测留痕、任务清单逐批勾选。
3. **探路优先**：含不确定性边界的探路工作必须作为首批 spike：结论留痕，代码不进入交付。
4. **验证档位**（受理时选定并留痕，依据与已拒绝替代见 ADR-002）：批内自测（永远启用）→ 组间快照 + 增量审查（冻结中间 `DevelopmentSnapshot`，复用 `ReviewScopeMode` 与 carry 继承判定：未受影响 PASS 直接沿用，只对增量 diff 审查/执行；组间轮不计 repair wave）→ 单次全量门禁 → split 多 run。档位依据结构性信号：预期生产逻辑是否多模块扩散、独立验收单元数；不依据总行数或批数。
5. **不变量**：批次不改变 run 边界、公共入口、Seal 语义或强制并行不变量；不产生额外受理、双审或 Seal 轮次。
6. **引擎义务**：以上判定全部由引擎机械承载——批次计划登记、组间快照与增量 diff artifact、carry/AFFECTED 判定复用既有机制；不新增自由编排面，不依赖代理自觉遵守。
