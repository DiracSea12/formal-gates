...
QA 覆盖契约的独立 fixtures 还必须证明：合法 source→point→case 多对多映射、无适用 QA 点的确认分类与替代验证 PASS 证据、未知/重复/缺失/orphan source/point/case 和集合级 PASS 被拒绝、source inventory/manifest/map/whitelist digest 或当前 `ValidationCandidate` 不匹配时旧结果失效、`FULL` expected/actual 不一致与 `AFFECTED` 执行/继承/未执行集合不完整被拒绝，以及 `AUTHORIZED_SKIP` 不计为执行 PASS。该 checkpoint 只冻结契约与 validator，regular E2E 留在阶段 4。
