---
name: formal-gates
description: Use when the user asks for 四门流程, writing-document gates, requirement clarification before documentation, formal gate review, OpenSpec/PRD/SDD/start-readiness review, release/seal validation, QA gate, complexity gate, architecture-health gate, code-quality gate, GateWorkflow, gate-state/gate-workflow hooks, zero-context gate review, or formal-gates AB testing.
---
...
普通聊天、头脑风暴、轻量解释不要启动这个 skill。只有用户要正式审查、正式开工判断、正式发布/封板结论，或明确提到四门、gate、OpenSpec 审查时才用。

## 先判定是哪一种流程

- 文档/开工审查：OpenSpec、PRD、SDD、设计文档、阶段文档、需求澄清、能不能开工。读 `references/document-writing-gates.md`。
...
OpenSpec proposal/design/spec/tasks、PRD、SDD、阶段计划、开工判断，先读 `references/document-writing-gates.md`。
