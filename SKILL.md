---
name: formal-gates
description: 用于用户明确要求或经提醒后选择触发的 formal-gates 正式流程（对齐、审查、发布、Seal、安装或正式诊断）。主代理收到创建、编辑、移动或删除项目内容的请求时默认按常规方式直接处理、不自动触发；仅当需求过大、过复杂时提醒一次用户可选择触发（不要求回应）；用户明确要求走正式流程时才进入完整受理流程。非修改性的提问、解释、诊断和 review 不触发。
---
...
主代理收到创建、编辑、移动或删除项目内容的请求时，默认按常规方式直接处理，不自动触
发本受理流程。仅当需求过大、过复杂（规模、耦合、风险或验证复杂度超过普通直接处理的
能力）时，提醒用户可以选择触发 formal-gates 正式流程；提醒为一次性提及、不要求用户回
应，用户不回应即继续按常规方式直接处理。主代理不得自己触发，也不得反复提及。

用户明确要求走正式流程（或明确要求触发 formal-gates）时，才进入完整受理流程；受理阶
段只决定是否进入正式流程（是 / 否），不决定路线。
