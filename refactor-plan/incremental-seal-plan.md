...
各阶段开发按总需求 §8.5"开发分批与组间验证"与 ADR-002 执行：受理时产出批次计划并受审；探路工作首批 spike、结论留痕不进交付；批内轻量检查每步、全量测试批边界；验证档位按结构性信号选定并留痕。

引擎承载义务的阶段分配：批次计划登记与批次终态判定进阶段 1 任务模型；组间 `DevelopmentSnapshot` 只读投影进阶段 1，真实组间快照与增量审查随阶段 3 完整流程迁移交付（复用该阶段迁移的 ReviewScopeMode/carry/继承判定）；split 档位随阶段 4/5 分片生命周期交付。engine 建成前的过渡期由 legacy 快照/审查/carry 修复轮机制承载组间档。

本次重构的补充约束：

1. 各阶段收口批固定且独立，含隔离安装构建、installed binary 回归与 namespace disjoint proof（三环境模型要求），不得并入其他批。
2. 预期生产逻辑多模块扩散或独立验收单元 ≥ 2 的阶段，受理时必须评估组间增量审查档或 split，不得默认单次全量门禁硬扛；评估结论留痕。
3. 阶段候选的测试与实现同批推进，不留到收口批补写。
