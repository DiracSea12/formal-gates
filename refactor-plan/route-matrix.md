...
这三个入口属于 3.5a 的薄 CLI 适配层：只读取 JSON、调用 `internal/engine/coverage` 的纯函数并
输出 JSON，不写 workflow state、不调用 VCS、不修改 legacy QA。入口必须先在本矩阵登记，再进入
实现；成功返回 0，常见契约校验失败返回非零并输出稳定 JSON 错误。

| 入口 | runtime | 唯一 writer | 输入/输出 | 允许的状态变化 | 错误码 | 是否只读 | 阶段绑定 | 计划枚举 | 机器证据 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `coverage validate` | 3.5a coverage contract | 无（纯函数） | JSON contract → JSON validation result | 无 | 稳定契约校验错误 | 是 | 3.5a | 计划内 | 3.5a blackbox CASE-020..CASE-038 |
| `coverage project-whitelist` | 3.5a coverage contract | 无（纯函数） | JSON contract/review → JSON whitelist projection | 无 | 稳定投影/校验错误 | 是 | 3.5a | 计划内 | 3.5a blackbox CASE-020..CASE-038 |
| `coverage reconcile-execution` | 3.5a coverage contract | 无（纯函数） | JSON contract/execution → JSON reconciliation result | 无 | 稳定 execution 对账错误 | 是 | 3.5a | 计划内 | 3.5a blackbox CASE-020..CASE-038 |
...
| coverage（validate/project-whitelist/reconcile-execution） | 只读（stdin JSON → stdout JSON） | 无（`internal/engine/coverage` 纯函数） | 无（contract/review/execution 输入自带 binding/digest） | 无持久化 receipt | 无需恢复（纯校验/投影） | 只读契约验收；不写 workflow state、不调用 VCS、不修改 legacy QA | legacy | legacy | 计划未枚举 | cli `coverage_test.go`；3.5a coverage blackbox CASE-020..CASE-038 |
